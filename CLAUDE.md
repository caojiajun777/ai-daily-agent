...
当前版本：**v2.3** — 7 板块架构，资本动向独立板块，财经源（CNBC/Reuters/Bloomberg）入池，版本号感知聚类，X Cookie Context 泄漏修复。
...
6. 资本动向  cap=3  Capital Moves         ← 融资/财报/IPO/估值/收购
7. 产业风向  cap=4  Industry Watch        ← 政策/监管/并购/人事/合作
```

v2.3 将原来的"业界风向"拆分为"资本动向"和"产业风向"，解决财经/财报新闻被政策/人事挤占的问题。
...
- 信息源从 46 扩展到 52+（新增 CNBC、Reuters、@GeminiApp）

### X Cookie Context 泄漏修复（2026-05-21）

XCookieAdapter 的 `_get_page()` 每次调用创建新的 BrowserContext，但 `fetch()` 只关闭 Page 不关闭 Context，导致 24 个 X 源累积 24 个未关闭的 Context，浏览器资源逐渐耗尽最终卡死在 Windows IOCP `GetQueuedCompletionStatus`。

修复：
- `_get_page()` 改为返回 `(context, page)` 元组，`fetch()` 的 `finally` 块同时关闭两者
- 移除"代理失败后 fallback 直连"逻辑（GFW 环境直连 x.com 永远超时，白白浪费时间）

修复后 X 源抓取从 35s/源 降到 12-15s/源。

### v2.3 覆盖提升（2026-05-21）

针对热点新闻漏抓（如 Gemini 3.5 Flash 发布、财报数据缺失）做了三层改进：

**P0 — 消除根因（4 项）**
| # | 改动 | 效果 |
|---|------|------|
| 1 | `default.yaml` 新增 `@GeminiApp` X 源 | Google 产品发布直连一手渠道 |
| 2 | `event_clusterer.py` 保留版本号 `3.5` ≠ `3 5`，版本号感知去重 | "Gemini 3.5 Flash" 不再被合并到 "Gemini 2.5 Flash" |
| 3 | `event_scorer.py` `_title_overlap` 字符集→SequenceMatcher + `_is_meaningful_update` 减罚（-0.12→-0.04） | 新版本发布不再被历史去重误伤 |
| 4 | `curator.py`/`scorer.py`/`selector.py`/`editor.py`/`prompts.yaml` 添加 20+ 财报关键词（`earnings` `revenue` `财报` `营收` 等） | 财经新闻不再被 relevance filter 过滤 |

**P1 — 拓展覆盖（3 项）**
| # | 改动 | 效果 |
|---|------|------|
| 5 | `default.yaml` 新增 CNBC、Reuters RSS 源（各 5 条/天） | 财经新闻首次入采集池 |
| 6 | `final_selector.py` 板块 cap "业界风向" 4→5 | 多 1 个位置承载商业/政策新闻 |
| 7 | `event_scorer.py` `_reader_utility` +`_impact_scope` 增加产品发布(+0.15)、财经(+0.10) boost；Bloomberg/CNBC/Reuters 纳为 trusted_media | 财务/产品新闻评分不再偏低 |

**P2 — 结构优化（3 项）**
| # | 改动 | 效果 |
|---|------|------|
| 8 | `event_scorer.py` breaking news boost：3 源以上 ×1.12，2 源+官方源 ×1.06 | 多源确认的热点自动浮到前排 |
| 9 | `default.yaml` `candidate_top_k` 40→60，`final_max_items` 22→25 | 高峰日不再因截断丢失重要事件 |
| 10 | 7 板块拆分：`资本动向`（融资/财报/IPO）+ `产业风向`（政策/并购/人事），涉及 6 个文件同步更新 | 财报不再跟政策挤同一个板块 |

**覆盖效果**（2026-05-21 实测）：
- 旧版（v2.2）：约 30% 重大头条覆盖率，0 条财经/财报内容
- 新版（v2.3）：约 45% 覆盖率，资本动向板块 2-3 条（Anthropic 盈利、SpaceX IPO、软银投资），CNBC 源贡献了 SpaceX IPO 报道
- 剩余缺口：LLM ResearchEditor 对财经新闻的偏好仍偏低，CNBC/Reuters 的 10 条财经新闻采集到了但 LLM 选中的比例不高
- 核心改进：从"0 条财经"到"有独立财经板块"是从 0 到 1 的突破

## 当前状态

- v2.3 架构：ResearchEditor + Writer 均用 deepseek-chat，X 源 CI 直连
- 自定义 API 网关仅暴露 `deepseek-v4-flash` + `deepseek-v4-pro`（无 `deepseek-chat`），通过 `skip_model_check=True` 绕过
- 7 板块新分类（今日头条/模型前沿/工具与开源/论文精选/产品落地/资本动向/产业风向）
- 论文配额（min 5 篇）在 curator 和 final_selector 双路径保障
- CI 每日 12:00 CST 自动运行（`daily_run.yml`）
- 测试 CI 在 push/PR 时自动运行（`test.yml`）
- 52+ 个有效信息源：24 X/Twitter + 22 RSS + arXiv + AI Hot + 36kr + CNBC + Reuters
