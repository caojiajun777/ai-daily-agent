...
- 如果候选标记为 already_reported=true，且标题/摘要没有明确后续进展，只能给 secondary 或 skip。
    ...
    try:
        ...
        for evt in events:
            ...
            if rm is None:
                ...
                cap = 0.24 if _already_reported_without_followup(evt) else 0.45
                evt.rule_score = round(min(evt.rule_score, cap), 4)
            ...
            if _already_reported_without_followup(evt):
                final_score = min(final_score, 2.4)
...
def _candidate_lines(events: List[EventCluster]) -> List[str]:
    ...
    for evt in events:
        ...
        published = evt.latest_seen_at or evt.published_at or evt.first_seen_at or "unknown"
        urls = ", ".join(evt.source_urls[:3])
        lines.append(f"     published_at: {published}")
        lines.append(f"     source_urls: {urls}")
        lines.append(
            "     source_meta: "
            f"evidence_type={evt.primary_evidence_type or 'unknown'}; "
            f"confidence={evt.primary_confidence or 'unknown'}; "
            f"already_reported={str(bool(getattr(evt, 'already_reported', False))).lower()}"
        )
...
def _already_reported_without_followup(evt: EventCluster) -> bool:
    return bool(getattr(evt, "already_reported", False)) and not _is_material_followup(evt)


def _is_material_followup(evt: EventCluster) -> bool:
    text = f"{evt.canonical_title} {evt.summary}".lower()
